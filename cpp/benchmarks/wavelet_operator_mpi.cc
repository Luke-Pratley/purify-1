...
class WaveletOperatorMPIFullFixture : public ::benchmark::Fixture {
 public:
  WaveletOperatorMPIFullFixture(){};
  void SetUp(const ::benchmark::State& state) {
    m_imsizex = state.range(0);
    m_imsizey = state.range(0);
    b_utilities::update_comm(m_world);
    sopt::wavelets::SARA_MPI m_sara{
        std::make_tuple("Dirac", 3u, m_world), std::make_tuple("DB1", 3u, m_world),
        std::make_tuple("DB2", 3u, m_world),   std::make_tuple("DB3", 3u, m_world),
        std::make_tuple("DB4", 3u, m_world),   std::make_tuple("DB5", 3u, m_world),
        std::make_tuple("DB6", 3u, m_world),   std::make_tuple("DB7", 3u, m_world),
        std::make_tuple("DB8", 3u, m_world)};

    // Get the number of wavelet coefs
    n_wave_coeff = m_sara.size() * m_imsizey * m_imsizex;
    m_Psi = sopt::linear_transform<t_complex>(m_sara, m_imsizey, m_imsizex);
  }
...
BENCHMARK_DEFINE_F(WaveletOperatorMPIFullFixture, Forward)(benchmark::State& state) {
  // Benchmark the application of the operator
...
BENCHMARK_DEFINE_F(WaveletOperatorMPIFullFixture, Adjoint)(benchmark::State& state) {
  // Apply Psi to a temporary vector
  Vector<t_complex> const image = Vector<t_complex>::Ones(m_imsizey * m_imsizex);
  Vector<t_complex> wavelet_coeff = Vector<t_complex>::Zero(n_wave_coeff);
...
BENCHMARK_REGISTER_F(WaveletOperatorMPIFullFixture, Forward)
    // //->Apply(b_utilities::Arguments)
    ->RangeMultiplier(2)
    ->Range(128, 128 << 3)
    ->UseManualTime()
...
BENCHMARK_REGISTER_F(WaveletOperatorMPIFullFixture, Adjoint)
    // //->Apply(b_utilities::Arguments)
    ->RangeMultiplier(2)
    ->Range(128, 128 << 3)
    ->UseManualTime()
